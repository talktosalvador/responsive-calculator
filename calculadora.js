...
inRight.addEventListener('input', (event) => {
    ...
    mobileRight.textContent = `${(event.target.value / 390 * 100).toFixed(2)}vw`;
})

inBottom.addEventListener('input', (event) => {
    fullBottom.textContent = `${(event.target.value / 1440 * 100).toFixed(2)}vw`;
    deskBottom.textContent = `${(event.target.value / 1024 * 100).toFixed(2)}vw`;
    mobileBottom.textContent = `${(event.target.value / 390 * 100).toFixed(2)}vw`;
})
...
inWidth.addEventListener('input', (event) => {
    ...
    mobileWidth.textContent = `${(event.target.value / 390 * 100).toFixed(2)}vw`;
})

inHeight.addEventListener('input', (event) => {
    fullHeight.textContent = `${(event.target.value / 1440 * 100).toFixed(2)}vw`;
    deskHeight.textContent = `${(event.target.value / 1024 * 100).toFixed(2)}vw`;
    mobileHeight.textContent = `${(event.target.value / 390 * 100).toFixed(2)}vw`;
})
...
let calcResponsive = (sizeLongPlus, sizeDesktop, sizePortrait) => {
    ...
    console.log(`LONG    ${finalLong}vw    DESKTOP    ${finalDesktop}vw    PORTRAIT    ${finalPortrait}vw`)
}
// console.log(`************SIZE************`);
